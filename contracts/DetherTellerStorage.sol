...
contract DetherTellerStorage is Ownable {
  ...
  function setTellerPosition(
    ...
    if ((countryId != teller[_address].pos.countryId || postalCode != teller[_address].pos.postalCode) && isTeller(_address)) {
  ...
  function getTellerPositionRaw(address _teller) public view returns (uint lat, uint lng, bytes32 countryId, int32 postalCode) {
   ...
   * Reput
   */
  function setReput(uint amount, address _sender, address _receiver) public onlyOwner {
  ...
  /*function getTellerReputation(address _teller) internal view returns (TellerReputation) {
      return teller[_teller].rep;
  }*/
  ...
  // Misc
  function addBalance(address _address, uint _amount) public onlyOwner {
  ...
  /*function getTeller(address _teller) internal view returns (Teller) {
    return teller[_teller];
  }*/
  ...
  function releaseEth(address _receiver, uint _amount) public onlyOwner returns (bool) {
