...
contract('Zone', () => {
  ...
  it('correctly initializes  contract', async () => {
    ...
    // user1 is the zoneOwner and this  contract reprensent where he can operate
    // and contains some conditions (and could be extended )
    // A Zone simply an identifier which represent a zone + the contract of the zone
    const cscZoneContract = await Zone.new(geohash, user1, owner, { from: owner });
    ...
    const computedCsc = await cscZoneContract.compute(geohash, cscZoneContract.address);
