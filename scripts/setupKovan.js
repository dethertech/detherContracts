...
module.exports = async (callback) => {
  ...
  //
  await detherCore.setShopModerator(ceoDether);
  console.log('shop moderator should be => ', ceoDether, await detherCore.shopModerators.call(ceoDether));
  await detherCore.setTellerModerator(ceoDether);
  ...
  console.log('dth inside detherBank should be => ', detherToken.address, await bank.detherToken.call());
  ...
  await kycCertifier.addDelegate(ownerdeploy, 'kycDelegate');
  console.log('ownerdeploy should be a kyc delegate => ', await kycCertifier.isDelegate.call(ownerdeploy));
  ...
  await detherCore.setSellDailyLimit(1, web3.toHex('AU'), 1000);
  console.log('getSellDailyLimit tier1 AU should be => ', '1000', (await detherCore.getSellDailyLimit(1, web3.toHex('AU'))));
  await detherCore.setSellDailyLimit(2, web3.toHex('AU'), 5000);
  console.log('getSellDailyLimit tier2 AU should be => ', '5000', (await detherCore.getSellDailyLimit(2, web3.toHex('AU'))));
  ...
  await detherCore.openZoneShop(web3.toHex('AU'));
